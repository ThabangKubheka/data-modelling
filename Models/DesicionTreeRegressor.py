...
bbd_bank_model.fit(X_train, y_train)

# Train a Decision Tree Regressor with limited depth
bbd_bank_model_simple = DecisionTreeRegressor(random_state=1, max_depth=3)
bbd_bank_model_simple.fit(X_train, y_train)

# Predict probabilities for validation set
val_probabilities = bbd_bank_model.predict(X_val)

# Prepare the submission format
submission_df = pd.DataFrame({
    'TX_ID': bbd_bank_data_validation['TX_ID'],
    'TX_FRAUD': val_probabilities  # Use the predicted probabilities as the fraud scores
})

# Save the submission to a CSV file
submission_file_path = '../Results/zubmission.csv'
submission_df.to_csv(submission_file_path, index=False)

# Prepare data for visualization
validation_predictions = (val_probabilities > 0.5).astype(int)  # Threshold at 0.5 for binary classification
fraud_indices = validation_predictions == 1
normal_indices = validation_predictions == 0

fraud_data = X_val[fraud_indices]
normal_data = X_val[normal_indices]

# Scatter plot for fraud and normal transactions
plt.scatter(fraud_data['TX_AMOUNT'], fraud_data['TRANSACTION_GOODS_AND_SERVICES_AMOUNT'], color='red',
            label='Fraud Transactions')
plt.scatter(normal_data['TX_AMOUNT'], normal_data['TRANSACTION_GOODS_AND_SERVICES_AMOUNT'], color='blue',
            label='Normal Transactions')

plt.xlabel('Transaction Amount')
plt.ylabel('Transaction Goods and Services Amount')
